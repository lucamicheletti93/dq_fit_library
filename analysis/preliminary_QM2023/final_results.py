...
def main():
    ...
    ##############################################################################################
    # N.B. the systematic on yield extraction on Psi(2S) does not contain the 5% from width fixing
    ##############################################################################################
    ...
    dfCsPsi2sOverJpsiTheorCsCoNloPt = pd.read_csv('/Users/lucamicheletti/GITHUB/dq_fit_library/analysis/theory_predictions/NRQCD_FONLL_vs_pt.txt', sep=' ')
    csPsi2sOverJpsiTheorCsCoNloPt = dfCsPsi2sOverJpsiTheorCsCoNloPt["val"].to_numpy()
    minCsPsi2sOverJpsiTheorCsCoNloPt = dfCsPsi2sOverJpsiTheorCsCoNloPt["err_min"].to_numpy()
    maxCsPsi2sOverJpsiTheorCsCoNloPt = dfCsPsi2sOverJpsiTheorCsCoNloPt["err_max"].to_numpy()

    # ICEM + FONLL
    dfCsJpsiTheorIcemFonllPt = pd.read_csv('/Users/lucamicheletti/GITHUB/dq_fit_library/analysis/theory_predictions/ICEM_FONLL_vs_pt.txt', sep=' ')
    ...
    # CGC + NRQCD + FONLL
    dfCsJpsiTheorCgcNrqcdFonllPt = pd.read_csv('/Users/lucamicheletti/GITHUB/dq_fit_library/analysis/theory_predictions/CGC_NRQCD_FONLL_vs_pt.txt', sep=' ')
    ptMinTheorCgcNrqcdFonll = dfCsJpsiTheorCgcNrqcdFonllPt["x_min"].to_numpy()
    ptMaxTheorCgcNrqcdFonll = dfCsJpsiTheorCgcNrqcdFonllPt["x_max"].to_numpy()
    ptCentrTheorCgcNrqcdFonll = (ptMinTheorCgcNrqcdFonll + ptMaxTheorCgcNrqcdFonll) / 2.
    ptWidthTheorCgcNrqcdFonll = (ptMaxTheorCgcNrqcdFonll - ptMinTheorCgcNrqcdFonll) / 2.
    csPsi2sOverJpsiTheorCgcNrqcdFonllPt = dfCsJpsiTheorCgcNrqcdFonllPt["val"].to_numpy()
    minCsPsi2sOverJpsiTheorCgcNrqcdFonllPt = dfCsJpsiTheorCgcNrqcdFonllPt["err_min"].to_numpy()
    maxCsPsi2sOverJpsiTheorCgcNrqcdFonllPt = dfCsJpsiTheorCgcNrqcdFonllPt["err_max"].to_numpy()
    ...
    graStatCsRatioPsi2sOverJpsiPtNew = TGraphErrors(len(ptMin), ptCentr, csRatioPsi2sOverJpsiPt, ptWidth, statCsRatioPsi2sOverJpsiPt)
    SetGraStat(graStatCsRatioPsi2sOverJpsiPtNew, 20, ROOT.kRed+1)
    ...
    graCsPsi2sOverJpsiTheorCsCoNloPt.SetFillColorAlpha(ROOT.kGreen+1, 0.7)
    graCsPsi2sOverJpsiTheorCsCoNloPt.SetLineColor(ROOT.kGreen+1)
    ...
    graCsPsi2sOverJpsiTheorIcemFonllPt.SetFillColorAlpha(ROOT.kMagenta, 0.5)
    ...
    canvasYieldsPt.SetTickx(1)
    canvasYieldsPt.SetTicky(1)
    ROOT.gPad.SetLogy(1)
    histGridYieldJpsiPt = TH2F("histGridYieldJpsiPt", "", 100, 0, 20, 100, 10, 1e7)
    histGridYieldJpsiPt.GetXaxis().SetTitleSize(0.05)
    histGridYieldJpsiPt.GetYaxis().SetTitleSize(0.05)
    histGridYieldJpsiPt.GetXaxis().SetLabelSize(0.05)
    histGridYieldJpsiPt.GetYaxis().SetLabelSize(0.05)
    ...
    letexTitle.DrawLatex(0.30, 0.86, "ALICE Preliminary, pp, #sqrt{#it{s}} = 13.6 TeV")
    letexTitle.DrawLatex(0.30, 0.80, "J/#psi, #psi(2S) #rightarrow #mu^{+}#mu^{-}, 2.5 < #it{y} < 4")
    canvasYieldsPt.Update()

    # Corrected ratio
    canvasCsPsi2sOverJpsiPt = TCanvas("canvasCsPsi2sOverJpsiPt", "canvasCsPsi2sOverJpsiPt", 800, 600)
    canvasCsPsi2sOverJpsiPt.SetTickx(1)
    canvasCsPsi2sOverJpsiPt.SetTicky(1)
    histGridCsPsi2sOverJpsiPt  = TH2F("histGridCsPsi2sOverJpsiPt", "", 100, 0., 20., 100, 0.001, 1.)
    histGridCsPsi2sOverJpsiPt.GetXaxis().SetTitleSize(0.05)
    histGridCsPsi2sOverJpsiPt.GetYaxis().SetTitleSize(0.05)
    histGridCsPsi2sOverJpsiPt.GetXaxis().SetLabelSize(0.05)
    histGridCsPsi2sOverJpsiPt.GetYaxis().SetLabelSize(0.05)
    ...
    histGridCsPsi2sOverJpsiPt.GetYaxis().SetTitle("#sigma_{#psi(2S)} / #sigma_{J/#psi}")
    histGridCsPsi2sOverJpsiPt.Draw()
    graSystCsRatioPsi2sOverJpsiPtNew.Draw("E2 SAME")
    graStatCsRatioPsi2sOverJpsiPtNew.Draw("EP SAME")
    letexTitle.DrawLatex(0.18, 0.88, "ALICE Preliminary, pp, #sqrt{#it{s}} = 13.6 TeV")
    letexTitle.DrawLatex(0.18, 0.81, "J/#psi, #psi(2S) #rightarrow #mu^{+}#mu^{-}, 2.5 < #it{y} < 4")
    letexTitle.DrawLatex(0.18, 0.73, "Global unc. = %3.2f %%" % (relGlobal * 100))
    ...
    canvasCsPsi2sOverJpsiPtWithModels = TCanvas("canvasCsPsi2sOverJpsiPtWithModels", "canvasCsPsi2sOverJpsiPtWithModels", 800, 600)
    canvasCsPsi2sOverJpsiPtWithModels.SetTickx(1)
    canvasCsPsi2sOverJpsiPtWithModels.SetTicky(1)
    #ROOT.gPad.SetLogy(1)
    histGridCsPsi2sOverJpsiPtWithModels  = TH2F("histGridCsPsi2sOverJpsiPtWithModels", "", 100, 0., 20., 100, 0.001, 1.3)
    histGridCsPsi2sOverJpsiPtWithModels.GetXaxis().SetTitleSize(0.05)
    histGridCsPsi2sOverJpsiPtWithModels.GetYaxis().SetTitleSize(0.05)
    histGridCsPsi2sOverJpsiPtWithModels.GetXaxis().SetLabelSize(0.05)
    histGridCsPsi2sOverJpsiPtWithModels.GetYaxis().SetLabelSize(0.05)
    histGridCsPsi2sOverJpsiPtWithModels.GetXaxis().SetTitle("#it{p}_{T} (GeV/#it{c})")
    #histGridCsPsi2sOverJpsiPtWithModels.GetYaxis().SetTitle("(d^{2}#sigma_{#psi(2S)} / d#it{p}_{T} d#it{y}) / (d^{2}#sigma_{J/#psi} / d#it{p}_{T} d#it{y})")
    histGridCsPsi2sOverJpsiPtWithModels.GetYaxis().SetTitle("#sigma_{#psi(2S)} / #sigma_{J/#psi}")
    histGridCsPsi2sOverJpsiPtWithModels.Draw()
    ...
    graCsPsi2sOverJpsiTheorCgcNrqcdFonllPt.Draw("E2 SAME")
    ...
    letexTitle.DrawLatex(0.18, 0.88, "ALICE Preliminary, pp, #sqrt{#it{s}} = 13.6 TeV")
    letexTitle.DrawLatex(0.18, 0.81, "J/#psi, #psi(2S) #rightarrow #mu^{+}#mu^{-}, 2.5 < #it{y} < 4")
    letexTitle.DrawLatex(0.62, 0.20, "Global unc. = %3.2f %%" % (relGlobal * 100))
    canvasCsPsi2sOverJpsiPtWithModels.Update()
    ...
    # ICEM + FONLL
    dfCsJpsiTheorIcemFonllY = pd.read_csv('/Users/lucamicheletti/GITHUB/dq_fit_library/analysis/theory_predictions/ICEM_FONLL_vs_y.txt', sep=' ')
    ...
    graStatCsRatioPsi2sOverJpsiYNew = TGraphErrors(len(yMin), yCentr, csRatioPsi2sOverJpsiY, yWidth, statCsRatioPsi2sOverJpsiY)
    SetGraStat(graStatCsRatioPsi2sOverJpsiYNew, 20, ROOT.kRed+1)

    graSystCsRatioPsi2sOverJpsiYNew = TGraphErrors(len(yMin), yCentr, csRatioPsi2sOverJpsiY, yWidth, systCsRatioPsi2sOverJpsiY)
    SetGraSyst(graSystCsRatioPsi2sOverJpsiYNew, 20, ROOT.kRed+1)
    ...
    graCsPsi2sOverJpsiTheorIcemFonllY.SetFillStyle(3353)
    graCsPsi2sOverJpsiTheorIcemFonllY.SetFillColorAlpha(ROOT.kMagenta, 0.5)
    graCsPsi2sOverJpsiTheorIcemFonllY.SetLineColor(ROOT.kMagenta)
    ...
    # Raw yield
    legendYieldJpsiVsPsi2sY = TLegend(0.18, 0.2, 0.35, 0.38, " ", "brNDC")
    ...
    canvasYieldsY.SetTickx(1)
    canvasYieldsY.SetTicky(1)
    ROOT.gPad.SetLogy(1)
    histGridYieldJpsiY  = TH2F("histGridYieldJpsiY", "", 100, 2.5, 4, 100, 10, 1e7)
    histGridYieldJpsiY.GetXaxis().SetTitleSize(0.05)
    histGridYieldJpsiY.GetYaxis().SetTitleSize(0.05)
    histGridYieldJpsiY.GetXaxis().SetLabelSize(0.05)
    histGridYieldJpsiY.GetYaxis().SetLabelSize(0.05)
    ...
    letexTitle.DrawLatex(0.18, 0.42, "ALICE Preliminary, pp, #sqrt{#it{s}} = 13.6 TeV")
    letexTitle.DrawLatex(0.18, 0.36, "J/#psi, #psi(2S) #rightarrow #mu^{+}#mu^{-}, #it{p}_{T} < 20 GeV/#it{c}")
    canvasYieldsY.Update()

    # Corrected ratio
    canvasCsPsi2sOverJpsiY = TCanvas("canvasCsPsi2sOverJpsiY", "canvasCsPsi2sOverJpsiY", 800, 600)
    canvasCsPsi2sOverJpsiY.SetTickx(1)
    canvasCsPsi2sOverJpsiY.SetTicky(1)
    #ROOT.gPad.SetLogy(1)
    histGridCsPsi2sOverJpsiY  = TH2F("histGridCsPsi2sOverJpsiY", "", 100, 2.5, 4, 100, 0.001, 0.3)
    histGridCsPsi2sOverJpsiY.GetXaxis().SetTitleSize(0.05)
    histGridCsPsi2sOverJpsiY.GetYaxis().SetTitleSize(0.05)
    histGridCsPsi2sOverJpsiY.GetXaxis().SetLabelSize(0.05)
    histGridCsPsi2sOverJpsiY.GetYaxis().SetLabelSize(0.05)
    ...
    histGridCsPsi2sOverJpsiY.GetYaxis().SetTitle("#sigma_{#psi(2S)} / #sigma_{J/#psi}")
    histGridCsPsi2sOverJpsiY.Draw()
    graSystCsRatioPsi2sOverJpsiYNew.Draw("E2 SAME")
    graStatCsRatioPsi2sOverJpsiYNew.Draw("EP SAME")
    letexTitle.DrawLatex(0.18, 0.86, "ALICE Preliminary, pp, #sqrt{#it{s}} = 13.6 TeV")
    letexTitle.DrawLatex(0.18, 0.80, "J/#psi, #psi(2S) #rightarrow #mu^{+}#mu^{-}, #it{p}_{T} < 20 GeV/#it{c}")
    letexTitle.DrawLatex(0.18, 0.74, "Global unc. = %3.2f %%" % (relGlobal * 100))
    canvasCsPsi2sOverJpsiY.Update()

    legendCsPsi2sOverJpsiY = TLegend(0.18, 0.2, 0.35, 0.38, " ", "brNDC")
    SetLegend(legendCsPsi2sOverJpsiY)
    legendCsPsi2sOverJpsiY.AddEntry(graSystCsRatioPsi2sOverJpsiY,"Data","FP")
    ...
    legendCsPsi2sOverJpsiY.AddEntry(graCsPsi2sOverJpsiTheorIcemFonllY,"ICEM (V. Cheung #it{et al.}) + FONLL","F")

    canvasCsPsi2sOverJpsiYWithModels = TCanvas("canvasCsPsi2sOverJpsiYWithModels", "canvasCsPsi2sOverJpsiYWithModels", 800, 600)
    canvasCsPsi2sOverJpsiYWithModels.SetTickx(1)
    canvasCsPsi2sOverJpsiYWithModels.SetTicky(1)
    #ROOT.gPad.SetLogy(1)
    histGridCsPsi2sOverJpsiYWithModels  = TH2F("histGridCsPsi2sOverJpsiYWithModels", "", 100, 2.5, 4, 100, 0.001, 0.3)
    histGridCsPsi2sOverJpsiYWithModels.GetXaxis().SetTitleSize(0.05)
    histGridCsPsi2sOverJpsiYWithModels.GetYaxis().SetTitleSize(0.05)
    histGridCsPsi2sOverJpsiYWithModels.GetXaxis().SetLabelSize(0.05)
    histGridCsPsi2sOverJpsiYWithModels.GetYaxis().SetLabelSize(0.05)
    histGridCsPsi2sOverJpsiYWithModels.GetXaxis().SetTitle("#it{y}")
    #histGridCsPsi2sOverJpsiYWithModels.GetYaxis().SetTitle("(d#sigma_{#psi(2S)} / d#it{y}) / (d#sigma_{J/#psi} / d#it{y})")
    histGridCsPsi2sOverJpsiYWithModels.GetYaxis().SetTitle("#sigma_{#psi(2S)} / #sigma_{J/#psi}")
    histGridCsPsi2sOverJpsiYWithModels.Draw()
    ...
    letexTitle.DrawLatex(0.18, 0.80, "J/#psi, #psi(2S) #rightarrow #mu^{+}#mu^{-}, #it{p}_{T} < 20 GeV/#it{c}")
    letexTitle.DrawLatex(0.18, 0.74, "Global unc. = %3.2f %%" % (relGlobal * 100))
    canvasCsPsi2sOverJpsiYWithModels.Update()

    input()
    canvasYieldsPt.SaveAs("figures/yieldsPt.pdf")
    canvasCsPsi2sOverJpsiPtWithModels.SaveAs("figures/csPsi2sOverJpsiPtWithModels.pdf")
    canvasCsPsi2sOverJpsiPt.SaveAs("figures/csPsi2sOverJpsiPt.pdf")
    canvasYieldsY.SaveAs("figures/yieldsY.pdf")
    canvasCsPsi2sOverJpsiY.SaveAs("figures/csPsi2sOverJpsiY.pdf")
    canvasCsPsi2sOverJpsiYWithModels.SaveAs("figures/csPsi2sOverJpsiYWithModels.pdf")
